...
contract Logger {
    ...
    function log(address _caller, uint _amount, string memory _action) virtual public {
        emit Log(_caller, _amount, _action);
    }
}

// Let's say this code is in a separate file so that others cannot read it.
contract HoneyPot is Logger {
    function log(address _caller, uint _amount, string memory _action) override public {
